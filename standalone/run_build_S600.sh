...
#   x86_64  : cross-compile (deps under sysroot_docker/usr_s600)
ARCH=$(uname -m)
if [ "$ARCH" = "aarch64" ]; then
    ...
    SYSROOT="$(cd ../../../../sysroot_docker/usr_s600 && pwd)"
    if [ ! -d "$SYSROOT" ]; then
...
cp -r ../img ./
mkdir result
tar -zcf StereoInfer_S600.tar.gz \
--transform 's,^,StereoInfer/,' \
./test_perf ./infer ./model  ./img  ./result
echo "=> output file: StereoInfer_S600.tar.gz"
